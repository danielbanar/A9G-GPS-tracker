 ...
 *          then register and login, add devices and the number is IMEI e.g. `
 `, finally the position of your device will be show in the map
 *
 ...
 *
 */
#define SERVER_IP "tutos.ddns.net"
    ...
    while (1)
    {
        if (isGpsOn)
        {
            ...
            snprintf(buffer, sizeof(buffer), "GPS fix mode:%d, BDS fix mode:%d, fix quality:%d, satellites tracked:%d, gps sates total:%d, is fixed:%s, coordinate:WGS84, Latitude:%f, Longitude:%f, unit:degree,altitude:%f", gpsInfo->gsa[0].fix_type, gpsInfo->gsa[1].fix_type,
                     gpsInfo->gga.fix_quality, gpsInfo->gga.satellites_tracked, gpsInfo->gsv[0].total_sats, isFixedStr, latitude, longitude, gpsInfo->gga.altitude.value);
            // show in tracer
            Trace(1, buffer);
            // send to UART1
            ...
            UART_Write(UART1, buffer, strlen(buffer));
            snprintf(requestPath, sizeof(buffer2), "/?id=%s&timestamp=%d&lat=%f&lon=%f&speed=%d&altitude=%d&batt=%.1f",
                     buffer, time(NULL), latitude, longitude, gpsInfo->vtg.speed_kph.value, gpsInfo->gga.altitude.value/10, percent * 1.0);
            uint8_t status;
